...
//Project Includes
#include <corvusoft/network/adaptor.hpp> //can be removed when we return EAGAIN.
...
namespace corvusoft
{
    ...
    namespace protocol
    {
        ...
        namespace detail
        {
            ...
            struct HTTPImpl
            {
                const std::function< std::error_code ( const std::shared_ptr< network::Adaptor >, const core::Bytes, const std::error_code, const std::shared_ptr< Message >, const std::function< std::error_code ( const std::shared_ptr< network::Adaptor >, const std::shared_ptr< Message >, const std::error_code ) > ) > parse = [ this ]( auto adaptor, auto data, auto status, auto message, auto completion_handler )
                {
                    if ( status or adaptor == nullptr ) return completion_handler( adaptor, nullptr, status );
                    
                    message->assemble( data );
                    if ( message->is_finalised( ) ) return completion_handler( adaptor, message, status ); //read until \r\n\r\n
                    
                    //once again just return EAGAIN removes these below.
                    const std::function< std::error_code ( const std::shared_ptr< network::Adaptor >, const core::Bytes, const std::error_code ) > consumption_handler = std::bind( parse, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, message, completion_handler );
                    adaptor->consume( consumption_handler );
                    
                    return status;
                };
                
                const std::function< std::error_code ( const std::shared_ptr< network::Adaptor >, const std::size_t, const std::error_code, const std::function< std::error_code ( const std::shared_ptr< network::Adaptor >, const std::error_code ) > ) > compose = [ this ]( auto adaptor, auto, auto status, auto completion_handler )
                {
                    return completion_handler( adaptor, status );
                };
